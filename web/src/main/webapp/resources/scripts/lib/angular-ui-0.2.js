 ...
 *
 * CUSTOM CHANGES:
 * Jaymes Bearden: See datetime directive
 */
...
/*
 ...
 // HACK: JAYMES BEARDEN: This has been updated to include the datetimepicker

 @param [ui-date] {object} Options to pass to $.fn.datetimepicker() merged onto ui.config
 */

angular.module('ui.directives').directive('uiDatetime', [
    'ui.config', function (uiConfig) {
        var options;
        options = {};
        if (uiConfig.datetime != null) {
            angular.extend(options, uiConfig.datetime);
        }
        return {
            require: '?ngModel',
            link: function (scope, element, attrs, controller) {
                var opts, updateModel, usersOnSelectHandler;
                opts = angular.extend({}, options, scope.$eval(attrs.uiDatetime));
                ...
                if (controller != null) {
                    ...
                    controller.$render = function () {
                        ...
                        return element.datetimepicker("setDate", date);
                    };
                }
                /* Create the datetimepicker widget
                 */

                return element.datetimepicker(opts);
            }
        };
    }
]);
